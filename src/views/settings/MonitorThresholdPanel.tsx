...
export function MonitorThresholdPanel() {
  ...
  return (
    <section className="settings-section">
      <div className="settings-section-title"><Icon name="monitor" size={15} /> 监控告警</div>
      <div className="section-tip">监控页轮询时检查当前服务器。超过阈值只在进入告警状态时通知一次。</div>
      ...
      <div className="threshold-grid">
        <label className="field">
          <span className="field-label">CPU 负载率 (%)</span>
        ...
        <label className="field">
          <span className="field-label">内存使用率 (%)</span>
        ...
        <label className="field">
          <span className="field-label">根分区使用率 (%)</span>
      ...
      <div className="settings-inline-actions">
        <button className="glass-btn" onClick={() => void saveMonitorThresholds()}><Icon name="save" size={14} />保存告警设置</button>
      </div>
    </section>
  )
}
