...
export function VaultPanel() {
  ...
  const submit = async () => {
    ...
    try {
      if (configured) {
        ...
        setNotice(t('凭据保险箱已解锁'))
      } else {
        await setup(password)
        setNotice(t('凭据保险箱已创建并解锁'))
      }
      setPassword('')
      setConfirm('')
    } catch (err) {
  ...
  const submitChangePassword = async () => {
    ...
    if (!oldPassword || !newPassword) {
      setChangeError(t('请填写原主密码与新主密码'))
      return
    }
    if (newPassword !== newConfirm) {
      setChangeError(t('两次输入的新主密码不一致'))
      return
    }
    if (newPassword === oldPassword) {
      setChangeError(t('新主密码不能与原主密码相同'))
      return
    }
    setChangeBusy(true)
    try {
      await changeMasterPassword(oldPassword, newPassword)
      setChangeNotice(t('主密码已更换，保险箱已使用新盐重新加密'))
      setOldPassword('')
      setNewPassword('')
      setNewConfirm('')
    } catch (err) {
  ...
  const deleteCredential = async (id: string) => {
    const accepted = await confirmDialog({
      title: t('删除保险箱凭据'),
      message: t('确定删除「') + hostName(id) + t('」保存的凭据吗？该主机下次连接时需要重新输入凭据。'),
      confirmLabel: t('删除'),
      danger: true
    })
    if (!accepted) return
  ...
  return (
    <div className="settings-section vault-panel">
      ...
      {!ready ? <div className="section-tip">{t('正在读取保险箱状态…')}</div> : unlocked ? (
        <>
          <div className="vault-status-row">
            <span><span className="status-dot" /> {t('保险箱已解锁')}</span>
            <button className="glass-btn" onClick={lock}>{t('锁定')}</button>
          </div>
          <div className="vault-credentials">
            <div className="vault-credentials-title">{t('已保存凭据（')}{credentialIds.length}{t('）')}</div>
            {!credentialIds.length ? (
              <div className="section-tip">{t('还没有保存任何凭据。连接主机时勾选自动保存即可写入保险箱。')}</div>
            ) : (
                  ...
                  return (
                    <div className="vault-cred-row" key={id}>
                      ...
                      <span className="vault-cred-kinds">
                        {credential.password ? <span className="vault-cred-kind">{t('密码')}</span> : null}
                        {credential.passphrase ? <span className="vault-cred-kind">{t('私钥口令')}</span> : null}
                        {!credential.password && !credential.passphrase ? <span className="vault-cred-kind">{t('空凭据')}</span> : null}
                      </span>
                      <button
            ...
            <div className="section-tip">{t('凭据内容不可查看，只能整体删除。删除对应主机会自动清理其凭据。')}</div>
          </div>
          <div className="vault-change-password">
            <div className="vault-credentials-title">{t('修改主密码')}</div>
            <label className="field">
              <span className="field-label">{t('原主密码')}</span>
            ...
            <div className="vault-change-grid">
              <label className="field">
                <span className="field-label">{t('新主密码')}</span>
                <input className="glass-input" type="password" value={newPassword} onChange={(event) => setNewPassword(event.target.value)} placeholder={t('至少 8 个字符')} autoComplete="new-password" />
              </label>
              <label className="field">
                <span className="field-label">{t('确认新主密码')}</span>
            ...
            <button className="glass-btn" onClick={() => void submitChangePassword()} disabled={changeBusy}>
              ...
              {changeBusy ? t('重新加密中…') : t('更换主密码')}
            </button>
            <div className="section-tip">{t('更换主密码会用新盐重新加密全部凭据，已有凭据内容保持不变。')}</div>
          </div>
        </>
      ) : (
        <>
          <label className="field">
            <span className="field-label">{configured ? t('主密码') : t('设置主密码')}</span>
            <input className="glass-input" type="password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder={t('至少 8 个字符')} />
          </label>
          {!configured && (
            <label className="field">
              <span className="field-label">{t('确认主密码')}</span>
          ...
          <button className="glass-btn primary" onClick={() => void submit()} disabled={busy}>
            ...
            {busy ? t('处理中…') : configured ? t('解锁保险箱') : t('创建保险箱')}
          </button>
        </>
      )}
      <label className="field vault-auto-lock">
        <span className="field-label">{t('闲置自动锁定')}</span>
        <select
        ...
        >
          {VAULT_AUTO_LOCK_OPTIONS.map((minutes) => (
            <option key={minutes} value={String(minutes)}>
              {t(AUTO_LOCK_LABELS[minutes] ?? `${minutes} 分钟`)}
            </option>
          ))}
        </select>
      </label>
      <div className="section-tip">{t('保险箱解锁后，若在设定时长内没有任何凭据操作，将自动锁定并清除内存中的明文凭据。')}</div>
      <label className="field checkbox-field">
        <span className="field-label">{t('窗口失焦时自动锁定')}</span>
        <input
          type="checkbox"
      ...
      <div className="section-tip">{t('开启后，只要应用窗口失去焦点（切换到其他窗口），保险箱立即锁定并清除内存凭据。关闭的窗口（如文件选择器）不会触发锁定。')}</div>
