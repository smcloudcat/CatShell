...
import { NetworkDiagnostic, ProcessInfo, ServerMetrics, STATUS_TEXT } from '../types/session'
...
export function MonitorView() {
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      setError(null)
      try {
        const next = await sshMonitor(selectedId)
        if (!cancelled) {
          setMetrics(next)
          ...
          for (const check of checks) {
            const alertKey = `${selectedId}:${check.key}`
  ...
  }, [selectedId, refreshKey, monitorThresholds])
  ...
  const killProcess = async (process: ProcessInfo) => {
    if (selectedId === null || !window.confirm(`确认终止进程 ${process.pid} (${process.name})？`)) return
    setProcessBusy(true)
    setProcessError(null)
    try {
      await sshKillProcess(selectedId, process.pid)
  ...
  const runDiagnostic = async () => {
    if (selectedId === null || !diagnosticTarget.trim()) return
    setBusy(true)
    setError(null)
    try {
      setDiagnostic(await sshNetworkDiagnostic(selectedId, diagnosticKind, diagnosticTarget))
    } catch (err) {
  ...
  const selected = selectedId === null ? null : sessions[selectedId]
  ...
  return (
    <div className="view monitor-view">
      <header className="view-header">
        <div>
          <div className="view-title">服务器监控</div>
          <div className="view-subtitle">通过 SSH 只读采集，不需要在服务器安装插件</div>
        </div>
        {selectedId !== null && (
      ...
      {connectedSessions.length === 0 ? (
        <section className="glass empty-state">
          <div className="empty-icon"><Icon name="monitor" size={44} /></div>
          <div className="empty-title">没有可监控的连接</div>
          <div className="empty-desc">请先在主机页面建立 SSH 连接，连接成功后即可采集服务器状态。</div>
        </section>
      ) : (
        <>
          <div className="monitor-toolbar glass">
            <Icon name="server" size={16} />
            <select className="glass-input monitor-select" value={selectedId ?? ''} onChange={(event) => setSelectedId(Number(event.target.value))}>
              {connectedSessions.map((session) => (
                <option key={session.id} value={session.id}>{session.name} · {session.host}:{session.port}</option>
              ))}
            </select>
            {selected && <span className="monitor-status">{STATUS_TEXT[selected.status]}</span>}
          </div>
              ...
              <section className="monitor-panels monitor-extra-panels">
                <div className="glass monitor-panel process-panel">
                  ...
                  <div className="process-list">
                    ...
                    {!processes.length && <span className="monitor-panel-meta">暂无进程数据</span>}
                  </div>
