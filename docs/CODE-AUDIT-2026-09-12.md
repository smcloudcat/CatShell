...
> **修复进度（2026-09-12）**：批次 1（P0/P1，提交 `1523162`，含随批落地的 B-12/B-13）与批次 2（正确性与体验，提交 `128a129`）已完成，累计 **27/56** 项已修复并落库；剩余 29 项（批次 3 / 批次 4）的清单见第八章。
...
3. **随后（性能与一致性）** — ⬜ 待修复
   ...
   - ⬜ B-10 幽灵 connecting、B-11 回放 auto 主题、B-14 续传指纹加短哈希
4. **择机（卫生）** — ⬜ 待修复
   - ⬜ R 组其余 12 项（悬空类名、拖放过滤、错误码判定、bidi、统一卸载守卫等；R-7 / R-9 已在批次 1 完成）
   - ⬜ X-8 ~ X-14（导入 busy、审计映射、误译、录制原子写、边界校验、known_hosts 阻塞 IO、托盘快照）

> 说明：批次 1 / 批次 2 均按单批独立验证并提交（`cargo fmt --check` / `clippy` / `test` + `tsc` / `eslint` / `vitest` / `i18n:check` / `mojibake:check` / `bindings:check` 全绿）。**累计 27 项已修复，剩余 29 项待处理**（批次 3 共 9 项、批次 4 共 20 项）。
