...
1. **立即（安全 / 数据丢失）** — 全部完成 ✅
   ...
   - ✅ B-3 递归删除拒绝 `.` / `..` 段
   - ✅ S-2 assetProtocol scope 收窄到专用背景目录（改为运行时 `allow_asset_file` 按需授权）
   - ✅ X-1 多跳私钥选择器修复（P1 功能失效，改动局部）
   - ✅ 顺带：R-7「关于」页英文词典、R-9 选图异常兜底
2. **本迭代（正确性与体验）** — 全部完成 ✅
   ...
   - ✅ 顺带：B-12 上传收尾字节校验、B-13 分片超时后禁止原位重试（随批次 1 一并完成）
3. **随后（性能与一致性）** — 全部完成 ✅
   - ✅ S-3 / X-5 发布与 CI 工作流整改：`TAURI_SIGNING_PRIVATE_KEY(_PASSWORD)` 从 job 级 `env` 降为仅签名 step 注入；`npm ci --ignore-scripts`；两工作流全部第三方 action 固定 commit SHA；补 `permissions`（最小授权）与 `timeout-minutes`
   - ✅ S-4 主加载路径校验：新增 `src/utils/settingsSchema.ts`（严格版 `normalizeTheme`/`isMonitorThresholds` 供备份导入、宽松版 `sanitizeTheme`/`sanitizeMonitorThresholds` 供启动加载逐字段回落），`settings.ts` 主路径与 localStorage 回退路径均改用它，`BackupPanel` 复用同一份严格实现
   - ✅ P-2 面板尺寸夹紧：`readSessionSize` 读取即按 `[min, max]` 收敛，`usePanelResize` 挂载与窗口 `resize` 时重新夹紧并把越界历史值写回
   - ✅ P-3 主题保存串行化：`settings.ts` 内所有对 `app-settings.json` 的写入共用一条 Promise 链（`writeSetting`），快速连点外观模式时最终落盘值即最后一次点击
   - ✅ P-7 会话恢复落盘判重改用派生 tabs 内容比较（不再按 `sessions` 引用），纯状态更新不再触发写盘
   - ✅ P-8 审计日志退出前 flush：新增 `flushAudit()`，接到 `onCloseRequested` 的两条真正退出路径上（隐藏到托盘无需干预）；⚠️ 托盘菜单「退出」走 Rust 侧 `app.exit(0)`，JS 侧无法拦截，仍可能丢 1s 内记录
